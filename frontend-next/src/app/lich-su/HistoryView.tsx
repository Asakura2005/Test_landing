...
import { getAboutUrl, getProductsPageUrl } from '@/utils/routeI18n'
...
const getSrc = (img: any): string => (typeof img === 'object' && img?.src ? img.src : img)
                ...
                <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 xl:gap-12 items-center">
                  ...
                  <div className={`w-full lg:col-span-7 ${isEven ? '' : 'lg:order-1'}`}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-2.5">
                      {chap.achievements.map((ach: string, aIdx: number) => (
...
/* ═══════════════════════════════════════════════════════════════════
   DATA — Chapters
   ═══════════════════════════════════════════════════════════════════ */
function getChapters(lang: string) {
  const en = lang === 'en'
  const ko = lang === 'ko'
  const zh = lang === 'zh'
  return [
    ...
    },
    {
      ...
      achievements: en ? [
        ...
        '与 WinMart、WinMart+、GO! 以及 Bach Hoa Xanh 签署全国直采供销协议。',
        '深度入驻 Circle K、GS25、K-Market 等知名 24 小时连锁便利系统。',
        '确立面向大型商业合作伙伴的大批量、持续稳定供应链履约能力。',
        '持续扩大全国各省市的分销网络与核心零售合伙人布局。',
      ] : [
        'Ký kết hợp đồng cung ứng toàn quốc với WinMart, WinMart+, GO!, Bách Hóa Xanh.',
        'Phủ sóng chuỗi cửa hàng tiện lợi 24/7 hàng đầu: Circle K, GS25, K-Market.',
        'Đảm bảo năng lực cung ứng sản lượng lớn, liên tục và ổn định cho các đối tác B2B.',
        'Mở rộng mạng lưới nhà phân phối và đại lý cấp 1 trên toàn quốc.',
      ],
      metric: en ? 'STORES' : ko ? '입점 매장' : zh ? '合作门店' : 'ĐIỂM BÁN',
      metricVal: '3.000+',
      image: b2bImg,
    },
    {
      ...
      theme: en ? 'GLOBAL EXPORT & STANDARDS' : ko ? '해외 수출 및 글로벌 인증' : zh ? '海外出口与国际认证' : 'TIÊU CHUẨN & XUẤT KHẨU',
      title: en ? 'ISO 22000 Certification & Official Export to South Korea, Taiwan'
        : ko ? 'ISO 22000 인증 획득 및 한국, 대만 정식 수출 개시'
        : zh ? '斩获 ISO 22000 认证与正规大宗出口韩国、中国台湾'
        : 'Đạt Chứng Nhận ISO 22000 & Xuất Khẩu Hàn Quốc, Đài Loan',
      lead: en ? 'Conquering the most stringent food safety benchmarks to open doors for international trade.'
        : ko ? '가장 까다로운 글로벌 식품안전 기준을 충족하며 국제 무역의 문을 활짝 열었습니다.'
        : zh ? '攻克严苛国际食品安全与检验检疫壁垒，全面开启海外主流出口贸易。'
        : 'Chinh phục những tiêu chuẩn khắt khe nhất về an toàn thực phẩm, mở đường cho hoạt động xuất khẩu chính ngạch.',
      desc: en ? 'Our manufacturing facility was audited and accredited with ISO 22000:2018 and HACCP CODEX. HAQ FOOD commenced official container shipments to South Korea and Taiwan, establishing our reputation on the global food stage.'
        : ko ? '생산 공장이 ISO 22000:2018 및 HACCP CODEX 국제 인증을 공식 취득했습니다. 한국과 대만으로 첫 정식 컨테이너 수출을 개시하여 글로벌 시장에서 품질 경쟁력을 입증했습니다.'
        : zh ? '现代化生产基地顺利通过 ISO 22000:2018 与 HACCP CODEX 双重国际食品认证。首批大宗集装箱正式出口韩国及中国台湾，在国际食品舞台彰显越南智造魅力。'
        : 'Hệ thống nhà máy được đánh giá và cấp chứng chỉ ISO 22000:2018 cùng HACCP CODEX. HAQ FOOD xuất khẩu chính ngạch thành công các lô hàng container đầu tiên sang Hàn Quốc và Đài Loan, khẳng định chất lượng thực phẩm Việt trên trường quốc tế.',
      achievements: en ? [
        'Officially awarded ISO 22000:2018 and HACCP CODEX food safety certificates.',
        'Exported official containers to major distributors in South Korea and Taiwan.',
        'Standardized multilingual export packaging and customs compliance labeling.',
        'Upgraded in-house QC laboratory for strict microbiological and humidity testing.',
      ] : ko ? [
        'ISO 22000:2018 및 HACCP CODEX 국제 식품안전 인증 공식 획득.',
        '한국 및 대만 대형 유통 바이어 대상 정식 컨테이너 수출 개시.',
        '다국어 수출용 패키징 및 국제 통관 라벨링 규격 표준화.',
        '로트별 미생물 및 수분율 정밀 검사를 위한 사내 품질관리(QC) 랩 고도화.',
      ] : zh ? [
        '荣获权威机构颁发 ISO 22000:2018 与 HACCP CODEX 国际食品安全认证。',
        '以正规海运集装箱形式批量出海，直接供货韩国及中国台湾主流渠道。',
        '全面对标国际海关标准，规范多语言外销包装与合规背标体系。',
        '升级厂区内部专业理化质检中心 (QC Lab)，批批严格检测微生物与水分。',
      ] : [
        'Được cấp chứng nhận ISO 22000:2018 và HACCP CODEX về an toàn thực phẩm.',
        'Xuất khẩu chính ngạch các đơn hàng lớn sang Hàn Quốc và Đài Loan.',
