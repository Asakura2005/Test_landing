...
function Reveal({ children, delay = 0, className = '' }: { children: React.ReactNode; delay?: number; className?: string }) {
  ...
  return (
    <div
      ref={ref}
      className={`transition-all duration-[900ms] ease-out ${v ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'} ${className}`}
  ...
  return (
    <div className="min-h-screen bg-white text-haq-ink font-sans flex flex-col">
      ...
      <main className="flex-1 pt-[72px] sm:pt-[76px]">
        ...
        {/* ──────────────────────────────────────────
            1. TIÊU ĐỀ TRANG & BỘ ẢNH NHÀ MÁY
        ────────────────────────────────────────── */}
        ...
        <section id="phan-phoi" className="bg-[#0C1E15] text-white py-20 sm:py-28 relative overflow-hidden">
          <img src={getSrc(distributionImg)} alt="" className="absolute inset-0 w-full h-full object-cover opacity-20" />
