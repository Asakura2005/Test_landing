 ...
 * and Schema.org rich snippet requirements for Next.js app.
 */
...
export function extractProductPrices(product: any, extraFallbackPrice: any = null): number[] {
  const prices: number[] = []
...
export function sanitizeDescription(text: any, fallback = ''): string {
...
export function generateProductSchema(product: any, options: ProductSchemaOptions = {}): any {
  ...
  const addImageCandidate = (img: any) => {
  ...
  if (Array.isArray(localizedProduct.variants)) localizedProduct.variants.forEach((v: any) => addImageCandidate(v?.img))
  if (Array.isArray(product.variants)) product.variants.forEach((v: any) => addImageCandidate(v?.img))
  ...
  const getCatName = (val: any): string => {
    if (!val) return ''
