...
const catAllImgSrc = catAllImg?.src || catAllImg
const catBanhTrangImgSrc = catBanhTrangImg?.src || catBanhTrangImg
const catBanhImgSrc = catBanhImg?.src || catBanhImg
const catDoAnVatImgSrc = catDoAnVatImg?.src || catDoAnVatImg
const catDoAnKhoImgSrc = catDoAnKhoImg?.src || catDoAnKhoImg
...
export function resolveProductImage(product, categorySlug = null) {
  if (!product) return catAllImgSrc
  ...
  if (categorySlug && CATEGORY_VISUALS[categorySlug]?.image) {
    const img = CATEGORY_VISUALS[categorySlug].image
    return img?.src || img
  }
  if (product.categories?.slug && CATEGORY_VISUALS[product.categories.slug]?.image) {
    const img = CATEGORY_VISUALS[product.categories.slug].image
    return img?.src || img
  }
  return catBanhTrangImgSrc
}
