...
const catBanhTrangSrc = catBanhTrangImg?.src || catBanhTrangImg
                                ...
                                <div className="pl-3.5 pr-1 py-1 flex flex-wrap gap-1.5">
                                  {cat.children.map((child) => {
                                    ...
                                    return (
                                      <Link
                                        key={child.id}
                                        href={getProductsPath(cat.slug, child.slug)}
                  ...
                  <div className="col-span-5 flex flex-col justify-between">
                    <div>
                      <div className="text-[11px] font-heading font-bold tracking-wider text-haq-green-dark uppercase mb-2 flex items-center justify-between border-b border-haq-border pb-1">
                        ...
                        <Link
                          href={getProductsPath(activePreviewCat?.slug || 'all')}
                        ...
                        {(() => {
                          ...
                          return filtered.slice(0, 4).map((p) => {
                            ...
                            return (
                              <Link
                                key={p.id}
                                href={language === 'en' ? `/en/products/${p.slug}` : language === 'ko' ? `/ko/products/${p.slug}` : language === 'zh' ? `/zh/products/${p.slug}` : `/san-pham/${p.slug}`}
                                onClick={() => {
                                ...
                                <div className="w-12 h-12 rounded-full overflow-hidden border border-haq-border shadow-2xs shrink-0 bg-white flex items-center justify-center">
                                  <img
                                    ...
                                    onError={(e) => {
                                      ...
                                      e.currentTarget.src = catBanhTrangSrc
                                    }}
                                  />
                                </div>
                ...
                <div className="grid grid-cols-1 gap-1 pt-1">
                  <Link
                    ...
                    className={`group block px-3.5 py-2.5 rounded-2xl transition-all ${
                      (pathname.startsWith('/tin-tuc') && !pathname.startsWith('/tuyen-dung')) || pathname.startsWith('/en/news') || pathname.startsWith('/ko/news') || pathname.startsWith('/zh/news')
                  ...
                  <Link
                    ...
                    className={`group block px-3.5 py-2.5 rounded-2xl transition-all ${
                      pathname.startsWith('/tuyen-dung') || pathname.startsWith('/en/careers') || pathname.startsWith('/ko/careers') || pathname.startsWith('/zh/careers')
                        ? 'bg-haq-sage/30 border border-[#16A34A]/20 shadow-2xs'
                        : 'hover:bg-haq-sage/20'
                    }`}
                  >
        ...
        <div className="flex lg:hidden items-center justify-between w-full h-full">
          ...
          >
            <div className="h-9 w-9 rounded-xl overflow-hidden border border-haq-border bg-white p-0.5 shadow-2xs shrink-0 flex items-center justify-center">
              <img src={logoSrc} alt="HAQ FOOD Logo" className="h-full w-full object-contain" />
            </div>
