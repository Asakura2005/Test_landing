...
export default function SearchOverlay({ isOpen, onClose }) {
  ...
  const handleSelect = (item) => {
    ...
    onClose()
    if (item.slug) {
      router.push(getProductDetailUrl(item.slug, language))
    } else {
      router.push(getProductsPageUrl(language))
    }
  }
              ...
              {results.length > 0 ? (
                ...
                <div className="text-center py-10 text-haq-text-secondary text-sm">
                  {language === 'en' ? (
                    ...
                    <>&ldquo;<strong>{query}</strong>&rdquo;에 대한 검색 결과가 없습니다.</>
                  ) : language === 'zh' ? (
                    <>未找到与 &ldquo;<strong>{query}</strong>&rdquo; 相关的匹配结果。</>
                  ) : (
                    <>Không tìm thấy kết quả phù hợp cho &ldquo;<strong>{query}</strong>&rdquo;.</>
                  )}
                </div>
              )}
            </div>
