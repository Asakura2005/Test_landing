...
export default function NewsDetailPage() {
  ...
  useEffect(() => {
    ...
    // 1. Meta Description (summary or excerpt)
    const rawSummary = localizedNews?.summary || news.summary || news.meta_description || news.content || ''
    const cleanDesc = rawSummary
      .replace(/<[^>]*>?/gm, '')
      .replace(/&[a-z0-9#]+;/gi, ' ')
      .replace(/\s+/g, ' ')
      .trim()
    ...
    const finalDesc = cleanDesc || `${articleTitle} — CÔNG TY CỔ PHẦN HAQ HÀ NỘI (HAQ FOOD). Tin tức và hoạt động doanh nghiệp.`
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({ title: localizedNews?.title || news?.title, url: window.location.href }).catch(() => {})
    } else {
      ...
      <main className="min-h-screen bg-[#FAF9F6] flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="font-mono text-xs uppercase tracking-widest text-[#52665A]">
            {language === 'en' ? 'Loading article...' : language === 'ko' ? '기사 불러오는 중...' : language === 'zh' ? '正在加载文章...' : 'Đang tải bài viết...'}
          </span>
        </div>
      </main>
    )
    ...
    return (
      <main className="min-h-screen bg-[#FAF9F6] flex flex-col items-center justify-center p-6 text-center">
        ...
        >
          ...
          <span>{language === 'en' ? 'Back to news' : language === 'ko' ? '뉴스 목록으로' : language === 'zh' ? '返回资讯列表' : 'Quay lại trang tin tức'}</span>
        </Link>
      </main>
    )
  }
        ...
        <div className="mx-auto max-w-4xl px-4 sm:px-6 md:px-8">
            ...
            >
              ...
              <span>
                {news?.category === 'Tuyển dụng'
                  ? (language === 'en' ? 'Back to careers' : language === 'ko' ? '채용 목록으로' : language === 'zh' ? '返回招聘列表' : 'Quay lại danh sách tuyển dụng')
                  : (language === 'en' ? 'Back to news' : language === 'ko' ? '뉴스 목록으로' : language === 'zh' ? '返回资讯列表' : 'Quay lại danh sách tin tức')}
              </span>
            </Link>
            ...
            >
              {isCopied ? (
                <>
                  ...
                  <span className="text-[#0F5132] font-semibold">{language === 'en' ? 'Copied!' : language === 'ko' ? '복사됨!' : language === 'zh' ? '已复制！' : 'Đã sao chép!'}</span>
                </>
              ) : (
                <>
                  <Share2 className="w-3.5 h-3.5" />
                  <span>{language === 'en' ? 'Share' : language === 'ko' ? '공유' : language === 'zh' ? '分享' : 'Chia sẻ'}</span>
                </>
              )}
            </button>
          </div>
          ...
          <header className="mb-8 text-left border-b border-[#E2E8E4] pb-6">
            ...
            <div className="flex flex-wrap items-center gap-y-2 gap-x-4 text-xs text-[#52665A] font-mono">
              <span className="flex items-center gap-1.5 text-[#0F5132] font-semibold">
                ...
                {new Date(localizedNews?.published_at || news.published_at).toLocaleDateString(language === 'en' ? 'en-US' : language === 'ko' ? 'ko-KR' : language === 'zh' ? 'zh-CN' : 'vi-VN')}
              </span>
              <span>•</span>
              <span className="flex items-center gap-1.5">
                <User className="w-3.5 h-3.5 text-gray-400" /> 
                {localizedNews?.author || (language === 'zh' ? 'HAQ FOOD 官方媒体部' : language === 'en' ? 'HAQ FOOD Media Team' : language === 'ko' ? 'HAQ FOOD 미디어팀' : 'Ban Truyền thông HAQ FOOD')}
              </span>
              {(news.source_name || news.source_url) && (
                <>
                  ...
                  <span className="flex items-center gap-1.5">
                    ...
                    <span>{language === 'en' ? 'Source: ' : language === 'ko' ? '출처: ' : language === 'zh' ? '来源：' : 'Nguồn: '}</span>
                    {news.source_url ? (
                      <a 
                      ...
                      >
                        {news.source_name || (language === 'en' ? 'Original article' : language === 'ko' ? '원문 기사' : language === 'zh' ? '原文报道' : 'Bài viết gốc')}
          ...
          <div 
            ...
            dangerouslySetInnerHTML={{ 
              __html: DOMPurify.sanitize(localizedNews?.content || news.content || '', {
          ...
          {(news.source_name || news.source_url) && (
            <div className="mt-6 px-5 py-3.5 rounded-xl bg-white border border-[#E2E8E4] flex flex-wrap items-center justify-between gap-3 text-xs text-[#52665A]">
              <div className="flex items-center gap-2">
                ...
                <span>
                  {language === 'en' ? 'Citation source: ' : language === 'ko' ? '인용 출처: ' : language === 'zh' ? '文章来源与引用：' : 'Nguồn bài viết & trích dẫn: '}
                  <strong className="text-[#11261B] font-semibold">{news.source_name || (language === 'zh' ? '新闻与媒体' : 'Báo chí & Truyền thông')}</strong>
                </span>
              </div>
              {news.source_url && /^https?:\/\//i.test(news.source_url.trim()) && (
                <a
                  ...
                  className="text-[#0F5132] hover:underline font-semibold inline-flex items-center gap-1"
                >
                  <span>{language === 'en' ? 'Open original source' : language === 'ko' ? '원문 열기' : language === 'zh' ? '打开原文链接' : 'Xem nguồn gốc'}</span>
                  <ExternalLink className="w-3.5 h-3.5" />
                </a>
              )}
            </div>
            ...
            <div className="mt-8 p-6 sm:p-8 bg-[#EBF3EC]/60 border border-[#0F5132]/25 rounded-2xl">
              ...
              <p className="text-xs sm:text-sm text-[#52665A] leading-relaxed mb-4">
                {language === 'en'
                  ...
                  : language === 'zh'
                  ? '有意应聘者请将个人简历发送至邮箱：hr@haqfood.com（邮件主题：[应聘岗位 - 姓名]），或致电热线 0969 516 888 咨询。'
              ...
              >
                <span>{language === 'en' ? 'Send CV to HR' : language === 'ko' ? '이메일로 지원하기' : language === 'zh' ? '通过邮件投递简历' : 'Gửi CV ứng tuyển qua Email'}</span>
                <ExternalLink className="w-3.5 h-3.5" />
              </a>
            </div>
          )}
          ...
          <div className="mt-8 pt-6 border-t border-[#E2E8E4] flex items-center justify-between">
            <div className="flex items-center gap-2 text-xs font-mono text-[#52665A]">
              ...
              <span>{localizedNews?.category || CATEGORY_MAP[news.category]?.[language] || news.category}</span>
            </div>
            ...
            >
              <Share2 className="w-3.5 h-3.5" />
              <span>{language === 'en' ? 'Share article' : language === 'ko' ? '기사 공유' : language === 'zh' ? '分享本文' : 'Chia sẻ bài viết'}</span>
            </button>
          </div>
