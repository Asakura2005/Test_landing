  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[92vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        {/* Modal Header */}
        <div className="p-6 border-b border-haq-border flex items-center justify-between bg-haq-cream">
          <div>
            <h2 className="font-heading font-black text-xl text-haq-ink uppercase">
              {news ? 'Chỉnh sửa bài viết' : 'Viết bài tin tức mới'}
            </h2>
            <p className="text-xs text-haq-text-secondary mt-0.5">Trình soạn thảo trực quan tích hợp lưu trữ ảnh đám mây</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Title */}
            <div>
              <label className="block text-xs font-mono font-bold uppercase tracking-wider text-haq-text-secondary mb-2">
              ...
              <input 
                ...
                className="w-full bg-haq-cream border border-haq-border px-4 py-3 rounded-xl text-haq-ink font-bold text-sm focus:outline-none focus:border-haq-red transition-colors"
              />
            </div>
            ...
            <div>
              ...
              <input 
                ...
                className="w-full bg-haq-cream border border-haq-border px-4 py-3 rounded-xl text-haq-ink font-mono text-xs focus:outline-none focus:border-haq-red transition-colors"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-haq-cream border border-haq-border px-4 py-3 rounded-xl text-haq-ink font-semibold text-xs focus:outline-none focus:border-haq-red transition-colors"
              >
          ...
          <div>
            ...
            <textarea 
              ...
              className="w-full bg-haq-cream border border-haq-border px-4 py-3 rounded-xl text-haq-ink text-xs focus:outline-none focus:border-haq-red transition-colors resize-none"
            />
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              <div className="w-28 h-20 rounded-xl bg-haq-cream border border-haq-border overflow-hidden flex items-center justify-center flex-shrink-0">
                {formData.image_url ? (
                  <img src={formData.image_url} alt="Cover" className="w-full h-full object-cover" />
                ) : (
                  <ImageIcon className="w-6 h-6 text-haq-text-secondary/40" />
                )}
              </div>
              <div className="flex-1">
                <label className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-haq-cream hover:bg-haq-cream/60 text-haq-ink font-bold text-xs cursor-pointer transition-colors border border-haq-border">
                ...
                <p className="text-[11px] text-haq-text-secondary mt-1">Hỗ trợ JPG, PNG, WEBP (Tự động tải lên Supabase Storage)</p>
              </div>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="block text-xs font-mono font-bold uppercase tracking-wider text-haq-text-secondary">
                Nội dung chi tiết bài viết <span className="text-haq-red">*</span>
              </label>

              {/* Toolbar */}
              <div className="flex items-center gap-1 bg-haq-cream p-1 rounded-xl border border-haq-border">
                ...
                <span className="w-px h-4 bg-haq-border mx-0.5" />
                ...
                {/* Insert Image Button */}
                <label className="flex items-center gap-1 px-2 py-1 bg-haq-red text-white text-xs font-bold rounded-lg cursor-pointer hover:bg-haq-red/90 transition-colors">
            ...
            <div 
              ...
              className="w-full min-h-[300px] bg-white border border-haq-border p-5 rounded-2xl focus:outline-none focus:border-haq-red text-haq-ink text-sm leading-relaxed prose max-w-none shadow-inner overflow-y-auto"
              style={{ wordBreak: 'break-word' }}
            />
            <p className="text-[11px] text-haq-text-secondary mt-1.5">
          ...
          {/* Footer Actions */}
          <div className="pt-4 border-t border-haq-border flex items-center justify-end gap-3">
            <button 
              type="button" 
              onClick={onClose}
              className="px-6 py-3 rounded-xl bg-haq-cream hover:bg-haq-cream/60 border border-haq-border text-haq-ink font-bold text-xs uppercase tracking-wider transition-colors"
            >
              Hủy bỏ
            </button>
            <button 
              ...
              className="px-8 py-3 rounded-xl bg-haq-red hover:bg-haq-red/90 text-white font-heading font-extrabold text-xs uppercase tracking-wider transition-all shadow-md flex items-center gap-2"
            >
