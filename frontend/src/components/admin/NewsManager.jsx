...
export default function NewsManager() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      ...
      {/* Content */}
      <main className="flex-1 p-4 md:p-6 overflow-y-auto bg-haq-cream/30">
        {isLoading ? (
          ...
          <div className="bg-white border border-haq-border rounded-2xl p-16 text-center shadow-sm max-w-2xl mx-auto mt-10">
            <div className="w-20 h-20 bg-haq-cream rounded-full flex items-center justify-center mx-auto mb-6 border border-haq-border">
              <FileText className="w-10 h-10 text-haq-text-secondary/40" />
            </div>
            <h3 className="text-xl font-heading font-black text-haq-ink mb-3 uppercase">Chưa có bài viết nào</h3>
            <p className="text-haq-text-secondary mb-8 leading-relaxed">
              Hệ thống tin tức đã sẵn sàng. Hãy bắt đầu chia sẻ những hoạt động mới nhất của HAQ FOOD tới khách hàng và đối tác.
            </p>
            <button 
              onClick={openNewModal}
              className="bg-haq-red text-white px-8 py-3 rounded-xl font-bold hover:bg-haq-red/90 transition-all shadow-md active:scale-95"
            >
              + Viết bài đầu tiên
            </button>
          </div>
        ) : (
          <div className="bg-white border border-haq-border rounded-2xl shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-haq-cream border-b border-haq-border text-[11px] uppercase tracking-widest text-haq-text-secondary font-mono">
                ...
                <tbody className="divide-y divide-haq-border">
                  {news.map(item => (
                    <tr key={item.id} className={`hover:bg-haq-cream/30 transition-colors group ${item.is_pinned ? 'bg-red-50/30' : ''}`}>
                      <td className="p-4 text-center">
                        <button 
                          onClick={() => togglePin(item)}
                          className={`p-2 rounded-full transition-all ${item.is_pinned ? 'text-haq-red bg-red-100/50 shadow-sm' : 'text-haq-text-secondary/40 hover:text-haq-red'}`}
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-4">
                          <div className="w-16 h-12 rounded-lg bg-haq-cream overflow-hidden flex-shrink-0 border border-haq-border">
                            {item.image_url ? (
                              <img src={item.image_url} alt="" className="w-full h-full object-cover" />
                            ) : (
                              <div className="w-full h-full flex items-center justify-center">
                                <ImageIcon className="w-5 h-5 text-haq-text-secondary/40" />
                              </div>
                            )}
                          </div>
                          <div>
                            ...
                            <div className="flex items-center gap-3 mt-1 text-[10px] text-haq-text-secondary font-mono uppercase tracking-tight">
                      ...
                      <td className="p-4">
                        <span className="inline-block bg-haq-cream border border-haq-border px-2.5 py-1 rounded-full text-[10px] font-bold text-haq-text-secondary uppercase tracking-wider">
                          {item.category || 'Tin tức'}
                        </span>
                      </td>
                      <td className="p-4 text-xs text-haq-text-secondary font-mono">
